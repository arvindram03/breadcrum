...
public class ContactUtility {
	public static Uri getContactImage(String phoneNumber, Context context) {
		...
		Cursor cursor = mResolver.query(uri, new String[] {
				PhoneLookup.PHOTO_THUMBNAIL_URI, PhoneLookup.PHOTO_URI }, null,
				null, null);

		Uri contactImageUri = null;

		if (cursor.moveToFirst()) {
			String contactImageUriString = cursor.getString(cursor
					.getColumnIndex(PhoneLookup.PHOTO_URI));
			if (contactImageUriString != null)
				contactImageUri = Uri.parse(contactImageUriString);
			else {
				contactImageUriString = cursor.getString(cursor
						.getColumnIndex(PhoneLookup.PHOTO_THUMBNAIL_URI));
				if (contactImageUriString != null)
					contactImageUri = Uri.parse(contactImageUriString);
			}
		}
